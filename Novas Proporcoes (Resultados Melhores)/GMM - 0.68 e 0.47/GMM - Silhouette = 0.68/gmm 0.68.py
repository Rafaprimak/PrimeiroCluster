...
IQR = Q3 - Q1
df = df[~((df[['Gold', 'Total']] < (Q1 - 2 * IQR)) | (df[['Gold', 'Total']] > (Q3 + 2 * IQR))).any(axis=1)]
...
labels = gmm.fit_predict(df[['Gold_to_Total']])
...
with pd.ExcelWriter('resultados_olympics2024_gmm.xlsx') as writer:
...
plt.title('Clusters de Países por Proporção de Medalhas de Ouro (GMM)')
